...
# El cupo diario NO vive aquí: viene de plan_limits.py, que es la fuente única.
PLAN_LIMITS: dict[str, dict] = {
    "FREE":    {"jobs_per_cargo": 25,  "max_ubicaciones": 1,  "max_email": 5},
    "PRO":     {"jobs_per_cargo": 50,  "max_ubicaciones": 4,  "max_email": 15},
    "PREMIUM": {"jobs_per_cargo": 100, "max_ubicaciones": 10, "max_email": 25},
    "TRIAL":   {"jobs_per_cargo": 50,  "max_ubicaciones": 4,  "max_email": 15},
}
DEFAULT_LIMITS = PLAN_LIMITS["FREE"]


def _get_limits(plan: str) -> dict:
    base = PLAN_LIMITS.get((plan or "FREE").upper(), DEFAULT_LIMITS)
    return {**base, "max_postulaciones_dia": get_plan_limit(plan)}
...
def process_user(user: dict) -> dict:
    ...
    # Extraer texto del CV una vez por usuario y cachearlo en el dict
    if not user.get("CV_TEXTO"):
        _cv_url = user.get("CV_URL") or user.get("cv_url") or ""
        if _cv_url:
            try:
                _cv_raw = _extract_cv_text(_cv_url)
                user["CV_TEXTO"] = _cv_raw[:4000] if _cv_raw else ""
                print(f"[{nombre}] CV extraído: {len(user['CV_TEXTO'])} chars", flush=True)
            except Exception as _e:
                print(f"[{nombre}] No se pudo extraer CV: {_e}", flush=True)
                user["CV_TEXTO"] = ""
    ...
    def _save_aplicacion(job: dict, fuente: str, empresa: str = "", descripcion: str = "", qa: list = None) -> None:
        ...
        stored_desc = (
            "[QA]" + _json.dumps(qa, ensure_ascii=False)
            if qa else descripcion
        )[:5000]
        rows_to_save.append({
            ...
            for job in cargo_new:
                ...
                if fuente in ("trabajando", "chiletrabajos"):
                    ...
                    if success:
                        ...
                        job_qa          = (result.get("qa")          if isinstance(result, dict) else None) or None
                        _save_aplicacion(job, fuente, job_empresa, job_descripcion, qa=job_qa)

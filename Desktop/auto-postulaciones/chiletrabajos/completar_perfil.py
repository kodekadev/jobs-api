...
def _pw_abrir_seccion(page, texto: str):
    # Eliminar ads de Google que interceptan clicks
    try:
        page.evaluate("""
            document.querySelectorAll(
                'ins.adsbygoogle, iframe[id^="aswift"], iframe[title="Advertisement"], .adsbygoogle-noablate'
    ...
    try:
        ...
        # JS click para saltarse interceptación de ads y backdrops
...
def _pw_cerrar_panel(page, panel_loc):
    if panel_loc is None:
        return
    try:
        ...
        if close.count() > 0:
            page.evaluate("el => el.click()", close.element_handle())
    except Exception:
        pass
    # Esperar que el backdrop desaparezca antes de continuar
    try:
        page.wait_for_selector(".offcanvas-backdrop", state="detached", timeout=4000)
    except Exception:
        pass
...
def _pw_completar_perfil_chiletrabajos(user_id: str, user: dict) -> bool:
    ...
    try:
        ...
        with sync_playwright() as pw:
            ...
            # Zoom out apenas carga el dominio (Chrome guarda el zoom por dominio)
            for _ in range(4):
                page.keyboard.press("Control+Minus")
            print("  [cht-pw-perfil] Zoom → ~67% (Ctrl+- x4)")

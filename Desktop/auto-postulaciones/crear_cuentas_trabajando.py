"""
Crea cuentas en Trabajando.cl para todos los usuarios de Postula Fácil.

Ejecutar:
    python crear_cuentas_trabajando.py           # todos los usuarios
    python crear_cuentas_trabajando.py jobs8     # solo un usuario
"""
...
def _run():
    ...
    print(f"[trabajando] {len(all_users)} usuario(s)")
    ok, skip, fail = 0, 0, 0
    for user in all_users:
        ...
        print(f"\n{'='*55}\n  {nombre} ({uid})\n{'='*55}")
        try:
            if bq.get_portal_account(uid, "trabajando"):
                print(f"  [{uid}] ya tiene cuenta — saltar")
                skip += 1
                continue
            if crear_cuenta_trabajando(uid, user):
                ok += 1
            else:
                fail += 1
    ...
    print(f"\n[trabajando] OK={ok} skip={skip} fail={fail} / {len(all_users)}")

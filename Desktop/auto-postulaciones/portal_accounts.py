...
def _pick_institution_claude(original: str, suggestions: list[str]) -> int:
    ...
    try:
        ...
        return -1  # en error, no seleccionar nada — el caller usará fallback de texto
            ...
            try:
                ...
                for ac_sel in _AC_SELS_INST:
                    ...
                    if sugs:
                        ...
                        idx = _pick_institution_claude(institucion, texts)
                        if idx == -1:
                            break  # ninguna sugerencia es válida, intentar query alternativa
                        chosen = sugs[idx]
                        chosen.click()
                        print(f"  [pw-wiz] Institución elegida: '{texts[idx][:50]}'")
                        return True
            except Exception as e:
...
def _paso2_experiencia(driver: webdriver.Chrome, user: dict) -> None:
    ...
    for sel in sels3:
        ...
        if is_mes and fecha_inits == 0 and fecha_fins == 0:
            ...
            anos = [v for v in opts_vals if v.isdigit() and 2000 <= int(v) <= 2030]
            anio_actual = str(_dt.date.today().year)
            fallback = anio_actual if anio_actual in anos else (
                max((v for v in anos if int(v) <= _dt.date.today().year), default=anos[0]) if anos else "2023"
            )
            objetivo = anio_fin if anio_fin and anio_fin in anos else fallback
            _select_by_value_safe(driver, sel, objetivo)
            ...
            try:
                ...
                for css in _AC_CSS:
                    ...
                    if sugs:
                        ...
                        idx = _pick_institution_claude(institucion, texts)
                        if idx == -1:
                            break  # ninguna sugerencia válida, intentar query alternativa
                        sugs[idx].click()
                        print(f"  [cv] Institución elegida: '{texts[idx][:50]}'")
                        return True
            except Exception as e:

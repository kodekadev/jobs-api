"""
Crea cuentas en Computrabajo para todos los usuarios de Postula Fácil.

Ejecutar:
    python crear_cuentas_computrabajo.py           # todos los usuarios
    python crear_cuentas_computrabajo.py jobs8     # solo un usuario
"""
...
def _run():
    ...
    print(f"[computrabajo] {len(all_users)} usuario(s)")
    ok, skip, fail = 0, 0, 0
    for user in all_users:
        ...
        print(f"\n{'='*55}\n  {nombre} ({uid})\n{'='*55}")
        try:
            if bq.get_portal_account(uid, "computrabajo"):
                print(f"  [{uid}] ya tiene cuenta — saltar")
                skip += 1
                continue
            if crear_cuenta_computrabajo(uid, user):
                ok += 1
            else:
                fail += 1
    ...
    print(f"\n[computrabajo] OK={ok} skip={skip} fail={fail} / {len(all_users)}")
